...
\item
  ...
  or
  \href{https://gitcdn.link/repo/cysouw/pandoc-ling/main/tests/readme_linguex.pdf}{Latex}.
\end{itemize}
...
\begin{samepage}
\ea \judgewidth{:–)} \label{ex:4.11} Completely superfluous preamble,
but it works \ldots{}
...
(\ref{ex:4.5}) after the processing of \texttt{pandoc-ling}. Needless to
...
(\ref{ex:4.11}\,c), smile! However, note that the ``c'' has to be
...
will work also, leading to (\ref{ex:4.11}\,haAl0) when formatted (which
