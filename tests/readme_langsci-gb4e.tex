...
\item
  ...
  \href{https://gitcdn.link/repo/cysouw/pandoc-ling/main/tests/readme.html}{HTML}
  or
  \href{https://gitcdn.link/repo/cysouw/pandoc-ling/main/tests/readme2_linguex.pdf}{Latex}.
\end{itemize}
...
\begin{samepage}
\ea \judgewidth{} \label{ex:4.3} Preamble\\
  This is an example with a preamble.
\z
\end{samepage}
...
\begin{samepage}
\ea \judgewidth{} \label{ex:4.5} Any nice description here
...
\begin{samepage}
\ea \judgewidth{whynot?} \label{ex:4.6} Throwing in a preamble for good
measure
...
\begin{samepage}
\ea \judgewidth{???} \label{ex:4.7} Here is a special case with a
preamble\\
...
\begin{samepage}
\ea \judgewidth{:-)} \label{ex:4.10} Completely superfluous preamble,
but it works \ldots{}
...
\texttt{{[}@test{]}}, leading to (\ref{test}) when formatted.
...
(\ref{ex:4.4}) after the processing of \texttt{pandoc-ling}. Needless to
...
(\ref{ex:4.10}\,c), smile! However, note that the ``c'' has to be
...
will work also, leading to (\ref{ex:4.10}\,haAl0) when formatted (which
...
\href{\%5Bhtt\%5D(https://github.com/cysouw/count-para)\%20for\%20a\%20practical\%20mechanism\%20to\%20add\%20such\%20numbering}{\texttt{count-para}}.
