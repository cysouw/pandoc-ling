...
\begin{samepage}
\pex[*=]<ex:5> Any nice description here\\
  \a one example sentence.
  \a two
  \a three
\xe
...
\begin{samepage}
\pex[*=whynot?]<ex:6> Throwing in a preamble for good measure\\
...
\begin{samepage}
\ex<ex:7> Here is a special case with a preamble\\
...
\begin{samepage}
\pex[*=:-)]<ex:10> Completely superfluous preamble, but it works
\ldots{}\\
  \a Mixing single line examples with interlinear examples.
...
\texttt{{[}@test{]}}, leading to (\getref{test}) when formatted.
...
(\getref{ex:4}) after the processing of \texttt{pandoc-ling}. Needless
...
(\getref{ex:10}\,c), smile! However, note that the ``c'' has to be
...
will work also, leading to (\getref{ex:10}\,haAl0) when formatted (which
