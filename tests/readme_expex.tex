...
\begin{samepage}
\pex[*=:–)]<ex:11> Completely superfluous preamble, but it works
\ldots{}\\
  \a Mixing single line examples with interlinear examples.
...
(\getref{ex:5}) after the processing of \texttt{pandoc-ling}. Needless
...
(\getref{ex:11}\,c), smile! However, note that the ``c'' has to be
...
will work also, leading to (\getref{ex:11}\,haAl0) when formatted (which
