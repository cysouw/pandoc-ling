...
\begin{samepage}
\pex[*=:–)]<ex:12> Completely superfluous preamble, but it works
\ldots{}\\
  \a Mixing single line examples with interlinear examples.
...
(\getref{ex:6}) after the processing of \texttt{pandoc-ling}. Needless
...
this: (\getref{ex:11}\,c), smile! However, note that the ``c'' has to be
