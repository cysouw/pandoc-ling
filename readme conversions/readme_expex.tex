...
\usepackage{expex}
\lingset{belowglpreambleskip=-1.5ex, aboveglftskip=-1.5ex, exskip=0ex, interpartskip=-0.5ex, belowpreambleskip=-1ex}
\ifluatex
...
\pex <ling-ex:4.6> Throwing in a preamble for good measure
  \a \ljudge{*} This traditionally signals ungrammaticality.
  \a \ljudge{?} Question-marks indicate questionable grammaticality.
  \a \ljudge{whynot?} But in principle any sequence can be used (here
even in superscript).
...
\ex <ling-ex:4.7> Here is a special case with a preamble\\
  \ljudge{???} With a singly questionably example. Note the alignment!
Especially with this very long example that should go over various lines
in the output.
\xe
...
some extra material in this example.  \a
  \begingl
  \glpreamble Dutch (Germanic) Note the grammaticality judgement!//
  \gla \emph{}\emph{Deze} \emph{zin} \emph{is}
...
\emph{header.}//
  \glb \textsc{dem} second sentence have.\textsc{3sg}.\textsc{pres} no header.//
...
e.g.~\texttt{{[}@Last{]}} will be formatted as ({\lastx}). By doubling
...
({\blastx}\,c), smile! However, note that the ``c'' has to be manually
...
also, leading to ({\blastx}\,Hall0) when formatted (which is of course
nonsensical).
