...
\ea \label{ling-ex:4.4} 
  \ea This is the first example.
  \ex This is the second.
  \ex The actual letters are not important, \texttt{pandoc-ling} will
put them in order.
  \ex Empty lines are allowed between labelled lines Subsequent lines
...
\ea \label{ling-ex:4.5} Any nice description here
...
\ea \label{ling-ex:4.6} Throwing in a preamble for good measure
  \ea *This traditionally signals ungrammaticality.
  \ex ?Question-marks indicate questionable grammaticality.
  \ex \textsuperscript{whynot?}But in principle any sequence can be used
(here even in superscript).
  \ex However, such long sequences sometimes lead to undesirable effects
in the layout.
  \z
\z
...
\ea \label{ling-ex:4.7} Here is a special case with a preamble\\
...
\ea \label{ling-ex:4.10} Completely superfluous preamble, but it works
\ldots{}
  \ea Mixing single line examples with interlinear examples.
  \ex This is of course highly unusal. Just for this example, let's add
some extra material in this example.

  \ex  Dutch (Germanic) Note the grammaticality judgement!\\
